  ...
  return (
    <View style={{
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        ...
        <Text style={{ fontSize: 15, color: colors.textTertiary }}>{label}</Text>
      </View>
      <TouchableOpacity
        onPress={onPress}
        style={{
        ...
        <Text style={{ fontSize: 15, fontWeight: '600', color: colors.textSecondary }}>{addLabel}</Text>
      </TouchableOpacity>
    </View>
  );
});
...
export const FieldLabel = React.memo(function FieldLabel({ label, colors }: FieldLabelProps) {
  return (
    <Text style={{ fontSize: 15, fontWeight: '500', color: colors.textSecondary, marginBottom: 6, marginTop: 14 }}>
      {label}
    </Text>
  );
});
