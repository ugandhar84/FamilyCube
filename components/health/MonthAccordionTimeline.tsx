...
import { getTypeCfg, safeISO, type TLEvent, type TLType } from '@/components/health/HealthUtils';
  ...
  return (
    <View style={{ paddingHorizontal: 16, paddingBottom: 8 }}>
        ...
        return (
          <View key={monthKey} style={{ marginBottom: 8, borderRadius: 14, borderWidth: StyleSheet.hairlineWidth, borderColor: isOpen ? accent + '40' : colors.border, backgroundColor: colors.card, overflow: 'hidden' }}>
            ...
            <TouchableOpacity onPress={() => toggleMonth(monthKey)} style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 12 }}>
              ...
              <Text style={{ flex: 1, fontSize: 14, fontWeight: '600', color: colors.textPrimary }}>{monthLabel}</Text>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <View style={{ paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10, backgroundColor: isOpen ? accent + '18' : colors.background }}>
                  <Text style={{ fontSize: 14, fontWeight: '600', color: isOpen ? accent : colors.textSecondary }}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, paddingHorizontal: 14, paddingBottom: 12 }}>
                {typeCounts.map(({ type, cfg, count }) => (
                  <View key={type} style={{ flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10, backgroundColor: cfg.bg, borderWidth: StyleSheet.hairlineWidth, borderColor: cfg.color + '30' }}>
                    ...
                    <Text style={{ fontSize: 14, color: cfg.color, fontWeight: '500' }}>
                  ...
                  return (
                    <View key={dateKey} style={{ borderBottomWidth: isLastDay ? 0 : StyleSheet.hairlineWidth, borderBottomColor: colors.border }}>
                      ...
                      <View style={{ paddingHorizontal: 14, paddingVertical: 7, backgroundColor: colors.background + 'CC', flexDirection: 'row', alignItems: 'center', gap: 7 }}>
                        ...
                        <Text style={{ fontSize: 14, fontWeight: '700', color: dayLabelColor, letterSpacing: 0.3 }}>
                      ...
                      {aiSummaryMap[dateKey] && (
                        <View style={{ marginHorizontal: 10, marginBottom: 6, backgroundColor: accent + '10', borderRadius: 10, padding: 10, flexDirection: 'row', gap: 6 }}>
                          ...
                          <View style={{ flex: 1 }}>
                            <Text style={{ fontSize: 14, fontWeight: '700', color: accent, letterSpacing: 0.4, marginBottom: 2 }}>FURAI SUMMARY</Text>
                            <Text style={{ fontSize: 14, color: colors.textPrimary, lineHeight: 17 }}>{aiSummaryMap[dateKey]}</Text>
                          </View>
                        </View>
                      )}
                        ...
                        return (
                          <View key={groupKey} style={{ borderTopWidth: ci === 0 ? 0 : StyleSheet.hairlineWidth, borderTopColor: colors.border }}>
                            <TouchableOpacity
                              ...
                              <View style={{ flex: 1 }}>
                                <Text style={{ fontSize: 15, fontWeight: '700', color: colors.textPrimary }}>{grp.cfg.label}s</Text>
                                <Text style={{ fontSize: 14, color: colors.textSecondary }}>{grp.items.length} records</Text>
                              </View>
                              <View style={{ paddingHorizontal: 7, paddingVertical: 2, borderRadius: 10, backgroundColor: grp.cfg.bg }}>
                                <Text style={{ fontSize: 14, fontWeight: '700', color: grp.cfg.color }}>{grp.items.length}</Text>
                              </View>
      ...
      {!hasOlder && monthGroups.length > 0 && (
        <Text style={{ textAlign: 'center', fontSize: 14, color: colors.textTertiary, paddingVertical: 8 }}>
...
export function SingleItemDetail({ ev, cfg, colors, onToggleMedActive }: SingleItemDetailProps) {
  if (ev.type === 'lab') {
    ...
    return (
      <>
        {explanation && (
          <Text style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 17, marginBottom: 5 }} numberOfLines={4}>
            {explanation}
          </Text>
        )}
        {value && (
          <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 6, marginTop: 2 }}>
            <Text style={{ fontSize: 15, fontWeight: '700', color: colors.textPrimary, fontFamily: 'monospace' }}>
            ...
            {ev.raw.is_abnormal === true && (
              <View style={{ backgroundColor: colors.dangerLight, borderRadius: 6, paddingHorizontal: 7, paddingVertical: 2 }}>
                <Text style={{ fontSize: 14, fontWeight: '800', color: colors.danger }}>ABNORMAL</Text>
              </View>
            )}
            {ev.raw.is_abnormal === false && (
              <View style={{ backgroundColor: colors.successLight, borderRadius: 6, paddingHorizontal: 7, paddingVertical: 2 }}>
                <Text style={{ fontSize: 14, fontWeight: '800', color: colors.success }}>NORMAL</Text>
              </View>
            )}
            {ev.raw.reference_range && (
              <Text style={{ fontSize: 14, color: colors.textTertiary }}>Ref: {ev.raw.reference_range}</Text>
            )}
          </View>
        )}
      </>
        ...
        {(ev.raw.dosage || ev.raw.frequency) && (
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 4 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              ...
              <Text style={{ fontSize: 14, color: cfg.color, fontWeight: '600' }}>
            ...
            <TouchableOpacity
              ...
              <Text style={{ fontSize: 14, fontWeight: '700', color: ev.raw.is_active ? colors.success : colors.textDisabled }}>
    ...
    return (
      <>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 2 }}>
          {ev.raw.manufacturer && <Text style={{ fontSize: 14, color: colors.textTertiary }}>{ev.raw.manufacturer}</Text>}
          {ev.raw.last_given && (
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 2, alignItems: 'center' }}>
          {ev.badge && (
            <View style={{
              ...
              <Text style={{ fontSize: 14, fontWeight: '800', color: ev.badgeColor ?? cfg.color, letterSpacing: 0.3 }}>
                {ev.badge}
              </Text>
            </View>
          )}
          {ev.clinic && (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
              ...
              <Text style={{ fontSize: 14, color: colors.textTertiary }}>{ev.clinic}</Text>
            </View>
          )}
        </View>
      </>
  ...
  if (ev.type === 'weight') {
    ...
    return (
      <Text style={{ fontSize: 20, fontWeight: '800', color: cfg.color, marginTop: 2 }}>
        {display} <Text style={{ fontSize: 14, fontWeight: '500', color: colors.textSecondary }}>{unitLbl}</Text>
      </Text>
    );
  }
  ...
  if (ev.type === 'allergy') {
    return (
      <>
        {ev.raw.notes && (
          <Text style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 17, marginBottom: 5 }} numberOfLines={3}>
            {ev.raw.notes}
          </Text>
        )}
        {ev.badge && (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 }}>
            <View style={{
              ...
              <Text style={{ fontSize: 14, fontWeight: '800', color: ev.badgeColor ?? cfg.color }}>
                {ev.badge.toUpperCase()}
              </Text>
            </View>
            {ev.raw.symptoms && (
              <Text style={{ fontSize: 14, color: colors.textSecondary }} numberOfLines={1}>{ev.raw.symptoms}</Text>
            )}
          </View>
        )}
      </>
