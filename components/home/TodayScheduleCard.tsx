      ...
      return (
        <View style={{ paddingHorizontal: 14, paddingBottom: 14 }}>
          <View style={{
            ...
            {rows.map((r, i) => (
              <View key={r.label} style={{
                ...
                <Text style={{ fontSize: 13, fontWeight: '700', color: colors.textTertiary, width: 68 }}>{r.label}</Text>
                <Text style={{ fontSize: 13, color: colors.textPrimary, flex: 1, fontWeight: '500' }}>{r.value}</Text>
              </View>
            ))}
          </View>
          <TouchableOpacity
          ...
          >
            ...
            <Text style={{ fontSize: 14, fontWeight: '700', color: AMBER }}>Mark as Given</Text>
          </TouchableOpacity>
        </View>
      );
    }
        ...
        {rows.length > 0 && (
          <View style={{
            ...
            {rows.map((r, i) => (
              ...
              >
                <Ionicons name={r.icon} size={14} color={accentColor} style={{ marginTop: 2 }} />
                <Text style={{ fontSize: 13, fontWeight: '700', color: colors.textTertiary, width: 56 }}>{r.label}</Text>
                <Text style={{ fontSize: 13, color: (r.label === 'Phone' || r.label === 'Address') ? accentColor : colors.textPrimary, flex: 1, fontWeight: '500' }}>{r.value}</Text>
  ...
  return (
    <View style={{
      ...
      <View style={{
        ...
        borderBottomWidth: 1,
        borderBottomColor: colors.border ?? (isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.06)'),
      }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          {items.length > 0 && (
            <View style={{
              ...
              <Text style={{ fontSize: 14, fontWeight: '700', color: TEAL }}>{items.length}</Text>
            </View>
          )}
        </View>
        <TouchableOpacity onPress={() => router.push('/health/all-schedules' as any)}>
          <Text style={{ fontSize: 14, fontWeight: '600', color: TEAL }}>See all ›</Text>
        </TouchableOpacity>
      </View>
      ...
      {items.length === 0 ? (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 10, paddingHorizontal: 14 }}>
          ...
          <Text style={{ fontSize: 14, fontWeight: '600', color: colors.textSecondary }}>All clear today</Text>
        </View>
      ) : (
        <>
          {items.slice(0, 5).map((item, idx) => {
            ...
            return (
              <View key={item.id} style={{ borderBottomWidth: isLast && !isExpanded ? 0 : 1, borderBottomColor: colors.border ?? (isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)') }}>
                ...
                >
                  <Text style={{ width: 44, fontSize: 14, fontWeight: '700', color: colors.textSecondary, fontVariant: ['tabular-nums'], flexShrink: 0 }}>
                    {item.timeLabel}
                  </Text>
                  ...
                  <View style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 2 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)', borderRadius: 8, paddingHorizontal: 5, paddingVertical: 1 }}>
                        <Text style={{ fontSize: 14 }}>{item.petEmoji}</Text>
                        <Text style={{ fontSize: 14, fontWeight: '600', color: colors.textSecondary }}>{item.petName}</Text>
                      </View>
                      <Text numberOfLines={1} style={{ fontSize: 14, color: colors.textSecondary, flex: 1 }}>{item.subtitle}</Text>
                    </View>
                  </View>
                  {item.isOverdue && item.type === 'appointment' && (
                    <View style={{ backgroundColor: CORAL + '20', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2, flexShrink: 0 }}>
                      <Text style={{ fontSize: 14, fontWeight: '800', color: CORAL, letterSpacing: 0.5 }}>OVERDUE</Text>
                    </View>
                  )}
