    ...
    <View style={{ position: 'absolute', top: 8, right: 8, width: 80, height: 70, alignItems: 'center', justifyContent: 'center' }}>
      ...
      {isClearNight && (
        <>
          ...
          <Animated.Text style={{ fontSize: 15, position: 'absolute', top: 4, right: 4,
            opacity: pulse.interpolate({ inputRange: [1, 1.15], outputRange: [0.4, 1] }) }}>✨</Animated.Text>
          <Animated.Text style={{ fontSize: 14, position: 'absolute', top: 20, right: 16,
            opacity: pulse.interpolate({ inputRange: [1, 1.15], outputRange: [1, 0.3] }) }}>⭐</Animated.Text>
          <Animated.Text style={{ fontSize: 14, position: 'absolute', top: 8, left: 10,
      ...
      {isCloudy && !isPartly && !isRainy && !isStormy && !isSnowy && (
        <>
          {isNight && (
            <Animated.Text style={{ fontSize: 24, position: 'absolute', top: 2, right: 6,
      ...
      {isRainy && !isStormy && (
        <>
          ...
          {([drop1, drop2, drop3] as Animated.Value[]).map((d, i) => {
            ...
            return (
              <Animated.Text key={i} style={{ fontSize: 15, position: 'absolute',
                top: 38, left: 4 + i * 26,
      ...
      {isStormy && (
        <>
          ...
          <Animated.Text style={{ fontSize: 24, position: 'absolute', top: 38, left: 14,
            opacity: flicker }}>⚡</Animated.Text>
          {([drop1, drop2] as Animated.Value[]).map((d, i) => {
            const { translateY, opacity } = dropInterp(d);
            return (
              <Animated.Text key={i} style={{ fontSize: 14, position: 'absolute',
                top: 30, left: 8 + i * 30,
                transform: [{ translateY }], opacity }}>
                💧
              </Animated.Text>
