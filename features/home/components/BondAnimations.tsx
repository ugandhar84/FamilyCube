  ...
  return (
    <View style={{ width: BOND_W, height: BOND_H, overflow: 'visible' }}>
      <View style={{
        ...
        {userUri
          ? <Image source={{ uri: userUri }} cachePolicy="memory-disk" style={StyleSheet.absoluteFillObject} contentFit="cover" />
      ...
      <View style={{
        ...
        <View style={{
          ...
          {petUri
            ? <Image source={{ uri: petUri }} cachePolicy="memory-disk" style={StyleSheet.absoluteFillObject} contentFit="cover" />
            : <View style={[StyleSheet.absoluteFillObject, { backgroundColor: petAccent, alignItems: 'center', justifyContent: 'center' }]}>
                <Text style={{ fontSize: PET_CS * 0.42, lineHeight: PET_CS * 0.5 }}>🐾</Text>
              </View>
          }
        </View>
      </View>
  ...
  return (
    <View style={{ alignItems: 'center', justifyContent: 'center', width: SIZE + 8, height: SIZE + 8 }}>
      ...
      <View style={{ alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: TYPO.micro, fontWeight: '700', color: sub, letterSpacing: 1, lineHeight: 12 }}>DAYS</Text>
        <Text style={{ fontSize: TYPO.micro, fontWeight: '700', color: fg, lineHeight: 12, marginTop: 1 }}>{label}</Text>
      </View>
    </View>
  );
}
...
function TimeTogether({ days, fg, sub }: { days: number; fg: string; sub: string }) {
  ...
  return (
    <View style={{ alignItems: 'center', justifyContent: 'center', width: BOND_W + 8, height: BOND_H + 8, overflow: 'hidden' }}>
      <Animated.Text style={{ fontSize: TYPO.title, transform: [{ scale: pulse }], lineHeight: 26 }}>❤️</Animated.Text>
      <View style={{ flexDirection: 'row', gap: 6, marginTop: 6 }}>
        {units.map((u, i) => (
          <View key={i} style={{ alignItems: 'center' }}>
            <Text style={{ fontSize: TYPO.heading, fontWeight: '900', color: fg, lineHeight: 20 }}>{u.val}</Text>
            <Text style={{ fontSize: TYPO.label, fontWeight: '700', color: sub, letterSpacing: 0.3 }}>{u.label}</Text>
          </View>
        ))}
      </View>
      <Text style={{ fontSize: TYPO.label, color: sub, marginTop: 4, letterSpacing: 0.8, fontWeight: '500' }}>TOGETHER</Text>
    </View>
  );
}

// ── ComeTogether ──────────────────────────────────────────────────────────────
function ComeTogether({ petUri, userUri, petAccent }: {
  petUri: string | null; userUri: string | null; petAccent: string;
  petName: string; fg: string; sub: string;
}) {
  const W = BOND_W + 8;
  ...
  const PhotoSquare = ({ tx, uri, accent, emoji }: {
    ...
    <Animated.View style={{
      ...
      {uri
        ? <Image source={{ uri }} style={{ width: CT_SIZE, height: CT_SIZE }} contentFit="cover" />
  ...
  return (
    <View style={{ width: W, height: BOND_H + 8, alignItems: 'center', justifyContent: 'center' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <PhotoSquare tx={petX}  uri={petUri}  accent={petAccent} emoji="🐾" />
      </View>
      <Animated.Text style={{
        position: 'absolute', fontSize: TYPO.heading,
        opacity: heart,
...
// ── SimpleOrbit ───────────────────────────────────────────────────────────────
function SimpleOrbit({ petUri, userUri, petAccent }: {
  petUri: string | null; userUri: string | null; petAccent: string;
}) {
  const anim = useRef(new Animated.Value(0)).current;
  ...
  const OrbitCircle = ({ tx, ty, uri, accent, emoji }: {
    ...
    <Animated.View style={{
      ...
      {uri
        ? <Image source={{ uri }} style={{ width: SIMPLE_SIZE, height: SIMPLE_SIZE }} contentFit="cover" />
  ...
  return (
    <View style={{ width: BOND_W + 8, height: BOND_H + 8, alignItems: 'center', justifyContent: 'center' }}>
      <Text style={{ fontSize: TYPO.heading, zIndex: 10 }}>🩷</Text>
      <OrbitCircle tx={uX} ty={uY} uri={userUri} accent="#9B6DD4" emoji="👤" />
      <OrbitCircle tx={pX} ty={pY} uri={petUri}  accent={petAccent} emoji="🐾" />
    </View>
  );
}
...
export const TogethernessBadge = memo(function TogethernessBadge(props: TogethernessBadgeProps) {
  const { days, petUri, userUri, petAccent, petName, fg, sub } = props;
  ...
  function renderView(idx: number) {
    switch (idx) {
      case 0: return <BondCircles petUri={petUri} userUri={userUri} petAccent={petAccent} />;
      case 1: return <MilestoneRing days={days} accent={petAccent} fg={fg} sub={sub} />;
      case 2: return <ComeTogether petUri={petUri} userUri={userUri} petAccent={petAccent} petName={petName} fg={fg} sub={sub} />;
      case 3: return <SimpleOrbit petUri={petUri} userUri={userUri} petAccent={petAccent} />;
      default: return null;
    }
  }
