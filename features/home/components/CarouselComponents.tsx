...
export const ProductCard = memo(function ProductCard({ p, CARD_W, colors, isDark, isActive, onPauseChange, onVideoEnd }: ProductCardProps) {
  ...
  return (
    <View style={[rc.cardShadow, { width: CARD_W }]}>
            ...
            <View style={{ width: '100%', height: '100%' }}>
              ...
              {videoEnded && (
                ...
                >
                  ...
                  <Text style={{ color: '#fff', fontSize: TYPO.body, fontWeight: '600', marginTop: 8, letterSpacing: 0.4 }}>Replay</Text>
                </TouchableOpacity>
              )}
              ...
              <TouchableOpacity style={rc.expandBtn} onPress={() => setLightboxOpen(true)} activeOpacity={0.8}>
      ...
      {/* Full-screen lightbox — image or video */}
      {(!!p.image_url || hasVideo) && (
        <Modal visible={lightboxOpen} transparent animationType="none" onRequestClose={() => setLightboxOpen(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.95)', alignItems: 'center', justifyContent: 'center' }}>
            {hasVideo ? (
              ...
              <TouchableOpacity activeOpacity={1} onPress={() => setLightboxOpen(false)} style={{ width: '92%', height: '62%' }}>
                <LazyImage uri={p.image_url!} style={{ width: '100%', height: '100%', borderRadius: 16 }} resizeMode="contain" />
              </TouchableOpacity>
            )}
            <View style={{ marginTop: 18, alignItems: 'center', gap: 4, paddingHorizontal: 24 }}>
              <Text style={{ color: '#fff', fontSize: TYPO.subheading, fontWeight: '700', textAlign: 'center' }} numberOfLines={2}>
                {p.brand ? `${p.brand} · ` : ''}{p.name}
              </Text>
              {p.price != null && (
                <Text style={{ color: col, fontSize: TYPO.body, fontWeight: '700' }}>${p.price.toFixed(2)}</Text>
              )}
            </View>
            {!!p.cta_url && (
              <TouchableOpacity
                onPress={() => { setLightboxOpen(false); Linking.openURL(p.cta_url!).catch(() => {}); }}
                style={{ marginTop: 20, backgroundColor: col, paddingHorizontal: 32, paddingVertical: 13, borderRadius: 28 }}
                activeOpacity={0.85}
              >
                <Text style={{ color: '#fff', fontWeight: '800', fontSize: TYPO.body }}>{p.cta_label}</Text>
              </TouchableOpacity>
            )}
            <TouchableOpacity
  ...
  return (
    <View style={{ marginBottom: 8 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, marginBottom: 10 }}>
        <Text style={{ fontSize: TYPO.body, fontWeight: '800', color: colors.textPrimary }}>Picked for {petName}</Text>
    ...
    >
      {ad.video_url ? (
        <View style={{ position: 'relative' }}>
          ...
          <TouchableOpacity style={sb.expandBtn} onPress={() => setLightboxOpen(true)} activeOpacity={0.8}>
            <Ionicons name="expand-outline" size={15} color="#fff" />
          </TouchableOpacity>
        </View>
      ) : ad.youtube_url ? (
        <View style={[sb.cover, { overflow: 'hidden' }]}>
          ...
          <View style={[sb.expandBtn, { pointerEvents: 'none' }]}>
            <Ionicons name="expand-outline" size={15} color="#fff" />
          </View>
        </View>
      ) : ad.image_url ? (
        <TouchableOpacity activeOpacity={0.9} onPress={() => setLightboxOpen(true)} style={sb.cover}>
          <LazyImage uri={ad.image_url} style={StyleSheet.absoluteFillObject} />
          <View style={sb.expandHint}>
            <Ionicons name="expand-outline" size={13} color="#fff" />
          </View>
        </TouchableOpacity>
      ) : (
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          {(ad as any).description ? (
            <>
              <TouchableOpacity onPress={() => setDescModalVisible(true)} activeOpacity={0.7}>
                ...
                <Text style={{ fontSize: TYPO.body, fontWeight: '700', color: accentCol, marginTop: 2 }}>Read more</Text>
              </TouchableOpacity>
              <Modal
              ...
              >
                ...
                >
                  <TouchableOpacity activeOpacity={1} style={{ backgroundColor: colors.card, borderRadius: 20, padding: 20, width: '100%', maxWidth: 360 }}>
                    <Text style={{ fontSize: TYPO.subheading, fontWeight: '800', color: colors.textPrimary, marginBottom: 10 }}>{ad.name}</Text>
                    <Text style={{ fontSize: TYPO.body, color: colors.textSecondary, lineHeight: 21 }}>{(ad as any).description}</Text>
                    <TouchableOpacity
                      onPress={() => setDescModalVisible(false)}
                      style={{ marginTop: 16, alignSelf: 'flex-end', paddingHorizontal: 18, paddingVertical: 8, borderRadius: 10, backgroundColor: accentCol }}
                    >
                      <Text style={{ fontSize: TYPO.body, fontWeight: '700', color: '#fff' }}>Close</Text>
                    </TouchableOpacity>
                  </TouchableOpacity>
                </TouchableOpacity>
              </Modal>
          ...
          {ad.city ? <Text style={[sb.city, { color: colors.textSecondary ?? colors.textSecondary }]}>📍 {ad.city}</Text> : null}
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <View style={[sb.cta, { backgroundColor: isDark ? accentCol : 'rgba(0,0,0,0.06)', borderColor: accentCol }]}>
            <Text style={[sb.ctaText, { color: isDark ? '#fff' : accentCol }]}>{ad.cta_label}</Text>
          </View>
          <TouchableOpacity
        ...
        <Modal visible={lightboxOpen} transparent animationType="none" onRequestClose={() => setLightboxOpen(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.95)', alignItems: 'center', justifyContent: 'center' }}>
            {ad.video_url ? (
              ...
              <TouchableOpacity activeOpacity={1} onPress={() => setLightboxOpen(false)} style={{ width: '92%', height: '62%' }}>
                <LazyImage uri={ad.image_url!} style={{ width: '100%', height: '100%', borderRadius: 16 }} resizeMode="contain" />
              </TouchableOpacity>
            )}
            <View style={{ marginTop: 18, alignItems: 'center', gap: 4, paddingHorizontal: 24 }}>
              <Text style={{ color: '#fff', fontSize: TYPO.subheading, fontWeight: '700', textAlign: 'center' }} numberOfLines={2}>{ad.name}</Text>
              {ad.subtitle ? <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: TYPO.caption, textAlign: 'center' }}>{ad.subtitle}</Text> : null}
            </View>
            {!!ad.cta_url && (
              <TouchableOpacity
                onPress={() => { setLightboxOpen(false); Linking.openURL(ad.cta_url!).catch(() => {}); }}
                style={{ marginTop: 20, backgroundColor: accentCol, paddingHorizontal: 32, paddingVertical: 13, borderRadius: 28 }}
                activeOpacity={0.85}
              >
                <Text style={{ color: '#fff', fontWeight: '800', fontSize: TYPO.body }}>{ad.cta_label}</Text>
              </TouchableOpacity>
            )}
            <TouchableOpacity
              onPress={() => setLightboxOpen(false)}
...
export const SponsoredBanner = memo(function SponsoredBanner({ ads, colors, isDark, visible = true }: SponsoredBannerProps) {
  ...
  return (
    <View style={{ marginBottom: 8 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, marginBottom: 10 }}>
        <Text style={{ fontSize: TYPO.body, fontWeight: '800', color: colors.textPrimary }}>Sponsored</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
          <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: colors.primary }} />
          <Text style={{ fontSize: TYPO.body, color: colors.textSecondary ?? colors.textSecondary }}>Ad</Text>
        </View>
      </View>
...
const rc = StyleSheet.create({
  cardShadow: { borderRadius: 18, shadowColor: '#000', shadowOpacity: 0.07, shadowOffset: { width: 0, height: 3 }, shadowRadius: 10, elevation: 2 },
  ...
  reason:   { fontSize: TYPO.body, marginTop: 1 },
  price:    { fontSize: TYPO.body, fontWeight: '800' },
  strike:   { fontSize: TYPO.body, textDecorationLine: 'line-through' },
  pctPill:  { borderRadius: 6, paddingHorizontal: 5, paddingVertical: 2 },
  pctText:  { fontSize: TYPO.body, fontWeight: '800' },
  cta:      { borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7, alignSelf: 'center', flexShrink: 0 },
  ctaText:  { fontSize: TYPO.body, fontWeight: '700', color: '#fff' },
  ...
  expandHint: { position: 'absolute', bottom: 8, left: 8, width: 26, height: 26, borderRadius: 13, backgroundColor: 'rgba(0,0,0,0.45)', alignItems: 'center', justifyContent: 'center' },
  expandBtn:  { position: 'absolute', top: 8, right: 8, width: 30, height: 30, borderRadius: 15, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' },
});
...
const sb = StyleSheet.create({
  ...
  sponsoredText:{ fontSize: TYPO.body, fontWeight: '800', color: '#fff', letterSpacing: 0.4 },
  body:         { flexDirection: 'row', alignItems: 'center', padding: 14, gap: 10 },
  name:         { fontSize: TYPO.body, fontWeight: '700' },
  sub:          { fontSize: TYPO.body, marginTop: 2 },
  city:         { fontSize: TYPO.body, marginTop: 3 },
