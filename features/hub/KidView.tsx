...
// Encode grocery request details into the kidRequest.detail field (human-readable + parseable)
...
// Grocery categories — School Supplies excluded (has its own dedicated form)
const GROCERY_CATEGORIES = ['Snacks', 'Produce', 'Dairy & Eggs', 'Pantry', 'Frozen', 'Bakery', 'Household', 'Other'];

// All grocery suggestions — searchable by name; category auto-set
const ALL_GROCERY_SUGGESTIONS: { name: string; emoji: string; category: string }[] = [
  ...
  { name: 'Rice Krispies',       emoji: '🟡', category: 'Snacks' },
  { name: 'Raisins',             emoji: '🍇', category: 'Snacks' },
  ...
  { name: 'Pita Chips',          emoji: '🥙', category: 'Snacks' },
  ...
  { name: 'Soup',                emoji: '🍲', category: 'Pantry' },
  { name: 'Ketchup',             emoji: '🍅', category: 'Pantry' },
  { name: 'Mayo',                emoji: '🫙', category: 'Pantry' },
  // Frozen
  ...
  { name: 'Frozen Burritos',     emoji: '🌯', category: 'Frozen' },
  { name: 'Edamame',             emoji: '🫛', category: 'Frozen' },
  // Beverages
...
function SuppliesModal({ visible, onClose, active, colors, isDark }: {
  visible: boolean; onClose: () => void;
  active: FamilyMember; colors: any; isDark: boolean;
}) {
  const { sendRequest } = useKidRequestStore();
...
// ─── Permission / question / medication bottom sheet ──────────────────────────
function AskModal({ visible, onClose, type, active, colors, isDark }: {
  visible: boolean; onClose: () => void;
  type: 'permission' | 'question' | 'medication';
  active: FamilyMember; colors: any; isDark: boolean;
}) {
  const { sendRequest } = useKidRequestStore();
...
export function KidView({ active, members, colors, isDark, onHelpRequest }: {
  ...
  return (
    <>
      ...
      <GroceryModal  visible={groceryModal}  onClose={() => setGroceryModal(false)}  active={active} colors={colors} isDark={isDark} />
      <SuppliesModal visible={suppliesModal} onClose={() => setSuppliesModal(false)} active={active} colors={colors} isDark={isDark} />
      {askModal && (
        <AskModal visible={!!askModal} onClose={() => setAskModal(null)} type={askModal} active={active} colors={colors} isDark={isDark} />
      )}
    </>
  );
}
