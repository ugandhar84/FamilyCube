/**
 * KioskAddEventForm — kiosk-only fork of EventFormModal.tsx's AddEventModal
 * (the "New Event" manual-entry form), same minimal-diff fork pattern as
 * KioskAddChoreForm.tsx/AddQuestModal.tsx and KioskSmartTaskComposer.tsx/
 * SmartTaskComposer.tsx: only the shell-owning import is swapped
 * (TaskFormShell -> KioskTaskFormShell, aliased to the same local name)
 * plus the relative `./components/eventForm/...` imports made absolute
 * (this file physically lives in a different directory) — every other
 * line of real logic (category/RBAC rules, kid ride requests, recurrence,
 * grocery links, assignment-suggestion engine, voice dictation) is
 * byte-identical [live-reported: "we must need same treatment when we
 * click for the add event button"]. EventFormModal.tsx/TaskFormShell.tsx
 * are completely untouched.
 *
 * The five PickerOverlay usages in this file are left exactly as-is —
 * PickerOverlay owns its own self-contained Modal (confirmed: no
 * dependency on TaskFormShell's frame), same category as MemberPicker/
 * PhotoRedactModal, so it renders correctly inside KioskTaskFormShell's
 * drawer body without needing the KioskDateTimePicker/openAndroidPicker
 * treatment other kiosk forms use for their own date fields.
 *
...
import { X } from '@/features/calendar/components/eventForm/Icons';
import Chip from '@/features/calendar/components/eventForm/Chip';
...
import GroceryLinkSection from '@/features/calendar/components/eventForm/GroceryLinkSection';
import CategoryFields from '@/features/calendar/components/eventForm/CategoryFields';
import HelperAssignmentSection from '@/features/calendar/components/eventForm/HelperAssignmentSection';
import { KidRideSection } from '@/features/calendar/components/eventForm/KidRideSection';
import { forkRideLegs } from '@/features/hub/parent/rideLegs';
import { f } from '@/features/calendar/components/eventForm/styles';
import {
...
export type { EventCategory } from '@/features/calendar/components/eventForm/types';
...
import AssignmentSuggestionCard from '@/features/calendar/components/eventForm/AssignmentSuggestionCard';
...
// AddEventModal
// ═══════════════════════════════════════════════════════════════════════════════
export function KioskAddEventForm({ visible, onClose, activeMemberId, prefill, initialStep }: {
  ...
  // step list (unlike AddQuestModal's conditional grocery step) — every
  ...
  //
  // Kiosk-only reduction from mobile's own 4-step What/When/Who/Review
  // flow down to 3: 'what' now also carries the real 'when' content (date/
  // time, all-day, repeats, call reminder, kid ride section) on the same
  // screen, matching KioskAddChoreForm.tsx's identical what+when merge —
  // kiosk's wide drawer has the room to show both without scrolling past a
  // single field group. EventFormModal.tsx's own AddEventModal is
  // untouched — this merge is chrome-only, no field/validation changed.
  const stepIds = ['what', 'assign', 'review'] as const;
  const [step, setStep] = useState(initialStep === 'review' ? stepIds.length - 1 : 0);
  type StepId = typeof stepIds[number];
  const currentStepId: StepId = stepIds[Math.min(step, stepIds.length - 1)];
  const stepTitles: Record<StepId, string> = {
    what: 'What & when', assign: 'Who & details', review: 'Review',
  };
  ...
  return (
    ...
    >
            ...
            {/* ── When it happens — merged onto this same step (kiosk-only
                 reduction, see stepIds comment above); mobile's own
                 AddEventModal keeps this as its separate 'when' step. ── */}
