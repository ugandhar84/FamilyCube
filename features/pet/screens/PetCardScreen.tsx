...
import { StatPill } from '@/features/pet/components/StatPill';
import { ir } from '@/features/pet/components/InfoRow';
...
// ── Detail row (card-specific, 96px label width) ──────────────────────────────
...
export default function PetCardScreen() {
  ...
  const [petRating, setPetRating] = useState<{ avg: number; count: number } | null>(null);
  ...
  useEffect(() => {
    ...
    fetchPet();
  }, [id, petFromStore]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (!id) return;
    supabase
      .from('pet_rating_stats')
      .select('avg_rating, total_ratings')
      .eq('pet_id', id)
      .maybeSingle()
      .then(({ data }) => {
        if (data?.avg_rating) setPetRating({ avg: Number(data.avg_rating), count: data.total_ratings });
      });
  }, [id]);
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        <View style={cs.statsRow}>
          ...
          {petRating && (
            <StatPill icon="⭐" value={petRating.avg.toFixed(1)} label={`${petRating.count} reviews`} />
          )}
        </View>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={[cs.qrCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <QRGrid value={shareUrl} size={92} color={ac} padding={4} borderRadius={12} />
