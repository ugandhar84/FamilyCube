...
export default function SmartTaskComposer({
  ...
  // Real, always-defined date/time to actually save — was previously only
  // computed for the "When" row's own DISPLAY (whenDate ?? fallback), while
  // every submit path (direct event/quest create, and the "open full form"
  // handoff) read raw detected.when.date/whenDate separately with no
  // fallback of their own. When neither the AI nor local detection caught a
  // date/time AND the user never touched the picker, that meant: events
  // silently saved as all-day (no time at all), quests saved with NO due
  // date whatsoever, and the "When" row itself was hidden (gated on
  // whenDate || detected.when.date || detected.when.time) so there was no
  // way to even notice or fix it before saving (live-reported: "when user
  // gives the create chore or create ride there AI or the smart tasker
  // could not get the date time, in that case we should default the date
  // to today and time current + next hr and provide date time picker to
  // allow user to choose"). Same fallbackTimeFor(today) rule the detection
  // effect above already uses for a bare time with no date.
  const resolvedWhen = whenDate ?? new Date(
  ...
  const create = guard(async () => {
    ...
    if (isRecurring || isRideWithReturn) {
      onOpenFullForm('event', {
        ...
        startAt: `${localDateStr(resolvedWhen)}T${String(resolvedWhen.getHours()).padStart(2, '0')}:${String(resolvedWhen.getMinutes()).padStart(2, '0')}:00`,
    ...
    if (isEvent) {
      // Always a real, defined date/time now (today + next-hour fallback
      // when nothing was detected/picked) — was `whenDate` directly, which
      // stayed null with nothing to fall back on whenever detection came
      // up empty, silently saving the event as all-day.
      const dt = resolvedWhen;
      ...
      const newQuest = await addQuest({
        ...
        // Always a real date+time now (today + next-hour fallback when
        // nothing was detected/picked) — was `detected.when.date ?? undefined`
        // with no dueTime at all, so a chore/ride whose date/time the AI or
        // local detection couldn't extract saved with NO due date
        // whatsoever (live-reported: "when user gives the create chore or
        // create ride there AI or the smart tasker could not get the date
        // time, in that case we should default the date to today and time
        // current + next hr and provide date time picker to allow user to
        // choose").
        dueDate: localDateStr(resolvedWhen),
        dueTime: `${String(resolvedWhen.getHours()).padStart(2, '0')}:${String(resolvedWhen.getMinutes()).padStart(2, '0')}`,
        photoRequired,
        createdById: activeMemberId,
        ...(alertCall ? { alertCall: true, alertCallLeadMinutes } : {}),
      });
            ...
            try {
              ...
              for (const store of allStores) {
                ...
                const { data: runRow, error: runErr } = await supabase
                  .from('grocery_runs')
                  .insert({ family_id: familyId, name: finalTitle, store: store === 'Any store' ? 'Store' : store, status: 'draft', created_by: activeMemberId, planned_at: localDateStr(resolvedWhen) })
                  .select('id').single();
                if (!runErr && runRow?.id) {
                  // Was a raw bulk insert into grocery_run_items — that table
                  // has no realtime subscription anywhere in the app, and
                  // this run was itself just created via a raw insert too
                  // (not createRun()), so useGroceryStore's own
                  // `runs[].runItems` never picked up these items until
                  // something happened to open the run's detail sheet.
                  // addItemToRun now self-heals via its own loadRunDetail()
                  // call, so routing through it here closes that gap instead
                  // of leaving the store silently stale.
      ...
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ gap: 14 }}>
        ...
        {showFields && (
          <>
            ...
            {/* Always shown now, for every category (Ride/Chore/any Event
                category alike) — was gated on whenDate || detected.when.date
                || detected.when.time, which hid this row entirely whenever
                neither the AI nor local detection caught a date/time,
                leaving no way to even notice (let alone fix) the event
                silently saving as all-day / the quest saving with no due
                date at all. Always resolves to a real value via
                resolvedWhen (today + next-hour fallback) and stays fully
                editable through the same picker (live-reported: "when user
                gives the create chore or create ride there AI or the smart
                tasker could not get the date time, in that case we should
                default the date to today and time current + next hr and
                provide date time picker to allow user to choose" / "in all
                categories from chore and the schedule forms"). */}
            <View style={{ gap: 6 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            ...
            <Pressable
              onPress={() => onOpenFullForm(isEvent ? 'event' : 'quest', {
                ...
                memberId: forMemberIds[0],
                // Always resolved now (today + next-hour fallback), same as
                // the direct-submit paths above — was raw detected.when.date
                // with no fallback, silently dropping the date/time on
                // handoff whenever nothing was detected.
                startAt: `${localDateStr(resolvedWhen)}T${String(resolvedWhen.getHours()).padStart(2, '0')}:${String(resolvedWhen.getMinutes()).padStart(2, '0')}:00`,
