...
export default function HealthFilterSheet({
  ...
  const accent = healthTab === 'meds' ? colors.danger : colors.teal;

  return (
    <AppBottomSheet
      visible={visible}
      onClose={onRequestClose}
      title={healthTab === 'meds' ? 'Medication Filters' : 'Vaccine Filters'}
      accentColor={accent}
      minHeight="60%"
      maxHeight="90%"
      footer={
        <View style={{ flexDirection: 'row', gap: 10 }}>
          <TouchableOpacity onPress={resetFilters}
            style={[hf.sheetHeaderBtn, { flex: 1, alignItems: 'center', borderColor: colors.primary + '50', backgroundColor: colors.primary + '10' }]}>
            <Text style={{ fontSize: 13, fontWeight: '800', color: colors.primary }}>Reset</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={applyFilters}
            style={[hf.sheetHeaderBtn, { flex: 2, alignItems: 'center', backgroundColor: accent, borderColor: 'transparent' }]}>
      ...
      {healthTab === 'meds' ? (
        <>
          ...
          <View style={hf.fsSection}>
            ...
            <View style={[hf.searchRow, { marginTop: 0, borderColor: colors.border, backgroundColor: isDark ? colors.card : colors.surface }]}>
          ...
          <View style={hf.fsSection}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {([
                ...
                { id: 'taken',   label: 'Taken Today', color: colors.success },
                { id: 'pending', label: 'Pending',  color: colors.primary },
                { id: 'overdue', label: 'Overdue',  color: colors.danger },
                { id: 'all',     label: 'All',      color: colors.info },
              ] as const).map(opt => {
          ...
          <View style={hf.fsSection}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {Object.entries(getCatColors(colors)).map(([cat, color]) => {
              ...
              {Object.entries(FREQ_LABELS).map(([k, v]) => {
                ...
                return (
                  <TouchableOpacity key={k}
                    ...
                    style={[hf.fsPill, { backgroundColor: sel ? colors.teal : 'transparent', borderColor: sel ? colors.teal : colors.border }]}>
        ...
        <>
          ...
          <View style={hf.fsSection}>
            <Text style={[hf.fsSectionTitle, { color: colors.textSecondary }]}>Search</Text>
            <View style={[hf.searchRow, { marginTop: 0, borderColor: colors.border, backgroundColor: isDark ? colors.card : colors.tealLight }]}>
          ...
          <View style={hf.fsSection}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {([
                { id: 'pending',  label: 'Pending',  color: colors.amber },
                { id: 'done',     label: 'Done',     color: colors.success },
                { id: 'due_soon', label: 'Due Soon', color: colors.danger },
                { id: 'all',      label: 'All',      color: colors.teal },
              ] as const).map(opt => {
      ...
      <View style={[hf.fsSection, { borderTopWidth: 1, borderColor: colors.border, paddingTop: 20 }]}>
        ...
        <View style={{ flexDirection: 'row', gap: 10 }}>
          ...
          <TouchableOpacity
            ...
            style={[hf.exportBtn, { borderColor: colors.teal + '60', backgroundColor: colors.teal + '10', flex: 1 }]}>
            <Share2 size={15} color={colors.teal} />
            <Text style={{ fontSize: 12, fontWeight: '800', color: colors.teal }}>Export / Share</Text>
          </TouchableOpacity>
        </View>
      </View>
    </AppBottomSheet>
