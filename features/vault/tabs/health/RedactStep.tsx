/**
 * RedactStep — the real drag-to-black-box redaction UI, extracted verbatim
 * out of ScanReviewSheet.tsx so it can be reused as its own untouched
 * component instead of copy-pasted or re-derived.
 *
 * [live-reported: "we can use same core logic of the mobile just the shell
 * we should use for the kiosk style component"] — the gesture handler
 * (Pan → box drawing), the ViewShot re-capture pipeline, and the per-image
 * box state are ALL genuinely tightly coupled to each other; this file is
 * that logic moved out whole, not rewritten, so both the phone's own
 * ScanReviewSheet and kiosk's new KioskScanReviewForm mount the exact same
 * real redaction feature rather than two copies that could drift (one of
 * them silently missing the privacy protection).
 *
 * All internal state (redactBoxesByImage, activeRedactIdx, currentBox, the
 * gesture ref plumbing, the animations) stays fully self-contained here —
 * only the inputs a caller genuinely needs to supply are lifted to props:
 * the scanner hook's own pendingImages/maxPhotos/pickImage/clearPending/
 * scan/scanning/scanError, plus onScan (called with the final,
 * possibly-redacted image list) and onDiscard.
 */
