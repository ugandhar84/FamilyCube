...
export default function ScanReviewSheet({
  ...
  // Redact-mode state (per-image boxes, the drag gesture, ViewShot capture)
  // now lives entirely inside RedactStep.tsx — extracted so kiosk's own
  // scan review can reuse the exact same real logic [live-reported: "we
  // can use same core logic of the mobile just the shell we should use
  // for the kiosk style component"].
  ...
  const closeScanSheet = () => {
    ...
    clearScan();
    setScanPage(1);
    // Redact box state now lives inside RedactStep.tsx, which unmounts
    // (pendingImages.length === 0) whenever clearScan()/clearPending()
    // runs, so its own local state resets naturally with it — nothing
    // left to reset here.
  };
  ...
  return (
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={closeScanSheet}>
      {/* ── REDACT MODE: full-screen layout — extracted verbatim into
          RedactStep.tsx [live-reported: "we can use same core logic of
          the mobile just the shell we should use for the kiosk style
          component"] so kiosk's own scan review can reuse the exact same
          real gesture/ViewShot redaction logic instead of a second copy
          that could drift or silently drop the privacy protection. ── */}
      {pendingImages.length > 0 && !scanning && (
        <RedactStep
          ...
          scanError={scanError}
          accent={scanMode === 'vaccine' ? colors.teal : colors.accent}
          onScan={scan}
        />
      )}
