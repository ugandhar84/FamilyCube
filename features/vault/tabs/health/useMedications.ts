...
export function useMedications(familyId: string | undefined, memberId: string | undefined) {
  ...
  const toggleMed = useCallback(async (med: Medication, time: string | null = null) => {
    ...
    // Was: computed the new taken_dates array (and the "all doses taken
    // today" derived taken_date) entirely client-side from `med`'s
    // possibly-stale snapshot, then blindly overwrote the whole column —
    // two people toggling different dose-times for the same medication
    // near-simultaneously could have whichever write landed second
    // silently clobber the first's entry. toggle_medication_dose does the
    // append/remove atomically against the row's CURRENT state in one
    // statement, so concurrent calls serialize instead of racing.
    const { data: newDates, error } = await supabase.rpc('toggle_medication_dose', {
      p_med_id: med.id, p_entry: entry, p_mark_taken: !wasTaken,
      p_today: todayStr, p_modified_by: memberId ?? null,
    });
    if (!error && newDates) {
      const timesForMed = med.frequency_times?.length ? med.frequency_times : ['08:00'];
      const allDosesTakenToday = timesForMed.every((t: string) => (newDates as string[]).includes(encodeTakenEntry(todayStr, timesForMed.length > 1 ? t : null)));
      setMeds(prev => prev.map(m => m.id === med.id
        ? { ...m, taken_date: allDosesTakenToday ? todayStr : null, taken_dates: newDates as string[], modified_by: memberId ?? null }
        : m));
