    ...
    <View style={{ marginBottom: 18 }}>
      ...
      <TouchableOpacity activeOpacity={0.85} onPress={() => setAiOpen(v => !v)}
        ...
        <View style={{ width: 28, height: 28 }}>
          <View style={{ width: 28, height: 28, borderRadius: 9, backgroundColor: colors.accent + '18', alignItems: 'center', justifyContent: 'center' }}>
            <ChefHat size={15} color={colors.accent} />
          </View>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
          <Text style={{ fontSize: 11, fontWeight: '700', color: colors.accent }}>{aiOpen ? 'Collapse' : 'Open'}</Text>
          {aiOpen ? <ChevronUp size={14} color={colors.accent} /> : <ChevronDown size={14} color={colors.accent} />}
        </View>
      </TouchableOpacity>
        ...
        <View style={{ gap: 12 }}>
              ...
              {['Kid-friendly', 'Vegetarian', 'High-protein', 'Under 20 min', 'Healthy', 'Weekend BBQ'].map(chip => (
                <TouchableOpacity key={chip} onPress={() => setAiPref(chip)}
                  style={{ borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6,
                    backgroundColor: aiPref === chip ? colors.accent + '22' : colors.accent + '10',
                    borderWidth: 1, borderColor: aiPref === chip ? colors.accent + '70' : colors.accent + '25' }}>
                  <Text style={{ fontSize: 11, fontWeight: '700', color: colors.accent }}>{chip}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </ScrollView>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 8, backgroundColor: isDark ? colors.surface : colors.background, borderRadius: 14, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 12, paddingVertical: 8 }}>
            ...
            <TouchableOpacity onPress={generateMealPlan} disabled={aiLoading || !aiPref.trim()}
              style={{ width: 32, height: 32, borderRadius: 10, backgroundColor: colors.accent, alignItems: 'center', justifyContent: 'center', opacity: aiPref.trim() ? 1 : 0.35 }}>
