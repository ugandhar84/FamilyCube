...
export default function MealSelectionPhase({
  ...
  return (
    <View style={{ marginBottom: 18 }}>
      ...
      {tip && (
        <View style={{ borderRadius: 12, backgroundColor: colors.teal + '15', borderWidth: 1, borderColor: colors.teal + '30', padding: 10 }}>
          <Text style={{ fontSize: 12, fontWeight: '600', color: isDark ? colors.teal : '#0F766E', lineHeight: 18 }}>💡 {tip}</Text>
        </View>
      )}
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 2 }}>
                    ...
                    {opt.dietaryTags.slice(0, 2).map(tag => (
                      <View key={tag} style={{ borderRadius: 5, paddingHorizontal: 5, paddingVertical: 1, backgroundColor: colors.teal + '20' }}>
                        <Text style={{ fontSize: 9, fontWeight: '700', color: colors.teal }}>{tag}</Text>
                      </View>
                    ))}
                  </View>
                </View>
      ...
      <TouchableOpacity onPress={confirmPlan} disabled={savingPlan}
        style={{ borderRadius: 14, paddingVertical: 13, alignItems: 'center', backgroundColor: colors.accent, flexDirection: 'row', justifyContent: 'center', gap: 8 }}>
        {savingPlan
