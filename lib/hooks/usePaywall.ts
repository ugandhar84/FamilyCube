import { useSubscriptionStore, SubscriptionTier } from '@/store/subscriptionStore';
import { useAuthStore } from '@/store/authStore';
import { usePetStore } from '@/store/petStore';
import { usePaywallSheetStore } from '@/store/paywallSheetStore';
import { canAccess, checkUsage, getLimit, FeatureKey, LIMITS } from '@/lib/subscription';

const TIER_RANK: Record<SubscriptionTier, number> = { free: 0, pro: 1, ultimate: 2 };

export function useContextTier(petId?: string | null): SubscriptionTier {
  const { tier } = useSubscriptionStore();
  const { petRoles, ownerTierByPet } = usePetStore();
  if (!petId || petRoles[petId] !== 'caretaker') return tier;
  const ownerTier = (ownerTierByPet[petId] ?? 'free') as SubscriptionTier;
  return TIER_RANK[ownerTier] > TIER_RANK[tier] ? ownerTier : tier;
}

export function usePaywall() {
  const { tier } = useSubscriptionStore();
  const { user } = useAuthStore();
  const showPaywall = usePaywallSheetStore(s => s.show);

  /** Returns true if the action is allowed. Opens PaywallSheet in-place if not. */
  const gate = async (
    feature: FeatureKey,
    opts?: { title?: string; message?: string },
  ): Promise<boolean> => {
    if (feature === 'pets') {
      const limit = LIMITS[tier].pets;
      if (limit === -1) return true;
      const { pets, petRoles } = usePetStore.getState();
      const currentCount = pets.filter(p => petRoles[p.id] === 'owner').length;
      if (currentCount >= limit) {
        showPaywall({
          headline: opts?.title ?? 'Pet limit reached',
          body: opts?.message ?? `Your plan supports ${limit} pet${limit === 1 ? '' : 's'}. Upgrade to add more.`,
        });
        return false;
      }
      return true;
    }

    if (!canAccess(tier, feature)) {
      showPaywall({
        headline: opts?.title ?? 'Pro feature',
        body: opts?.message ?? 'Upgrade to unlock this feature.',
      });
      return false;
    }

    const limit = getLimit(tier, feature);
    if (limit === -1) return true;
    if (!user?.id) return true;

    const { allowed, current } = await checkUsage(user.id, tier, feature);
    if (!allowed) {
      showPaywall({
        headline: opts?.title ?? 'Limit reached',
        body: opts?.message ?? `You've used ${current}/${limit} this month. Upgrade for unlimited access.`,
      });
      return false;
    }

    return true;
  };

  /** Increment usage after a successful action. */
  const consume = async (feature: FeatureKey) => {
    if (!user?.id) return;
    const limit = getLimit(tier, feature);
    if (limit === -1 || limit === 0) return;
    await useSubscriptionStore.getState().incrementUsage(user.id, feature);
  };

  return { gate, consume, tier };
}
