...
serve(async (req) => {
  ...
  try {
    ...
    // Live-reported/confirmed via direct DB query: retry_count was stuck at
    // 0 on EVERY call_reminder_log row ever written, across many real days
    // of usage. Root cause: this whole block used to sit AFTER the
    // `targets.length === 0` early return below (which fires whenever
    // nothing NEW happens to be due in this exact cron minute — the common
    // case, since reminders are spaced out) — meaning the follow-up check
    // only ever ran on a minute where some other unrelated reminder also
    // happened to be newly ringing. Moved above the early return so it runs
    // on every single invocation regardless of what else is due right now.
    //
    // A call reminder that rings and is never answered previously just
    // disappeared — the claim (below, once targets exist) permanently marks
    // that due_at as "rung" regardless of whether anyone picked up, and the
    // native side's TTS only ever speaks on an actually-connected call, so
    // an unanswered ring left nothing behind at all. answered is set true by
    // the client (via mark-call-reminder-answered) the moment
    // CXCall.hasConnected fires; anything still false here, ~3-4 minutes
    // after it first rang, was genuinely missed. Bounded to retry_count = 0
    // so this fires exactly once per reminder — a single quiet follow-up
    // (one more call attempt + one normal push notification), never a
    // repeating nag.
    ...
    if (targets.length === 0) return json({ ok: true, rung: 0, followedUp });
