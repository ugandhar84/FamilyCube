...
interface PriceResult {
  ...
  source: 'kroger' | 'estimate' | 'unknown';
}
...
async function estimatePricesAI(items: string[]): Promise<Record<string, number>> {
  const prompt = `You are a grocery price estimator. Given a list of grocery items, return a JSON object mapping each item name exactly to its estimated US retail price in USD (number only, no $ sign). Be realistic — use average supermarket prices. Include Indian/ethnic grocery items too.

Items: ${JSON.stringify(items)}

Return ONLY valid JSON like: {"chicken breast": 7.99, "basmati rice": 4.99, "paneer": 5.99}`;
  ...
  try {
    ...
    // hard fallback: flat $2.99
    return Object.fromEntries(items.map(i => [i, 2.99]));
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!isUS) {
      ...
      const prices = items.map(name => ({
        name, krogerPrice: null, unit: null, available: false,
        fallbackEstimate: aiPrices[name] ?? 2.99, source: 'estimate' as const,
      }));
    ...
    const results: PriceResult[] = krogerResults.length > 0
      ? krogerResults.map(r => {
          if (!r.krogerPrice && aiPrices[r.name] != null) {
            return { ...r, fallbackEstimate: aiPrices[r.name], source: 'estimate' as const };
          }
          return r;
        })
      : items.map(name => ({
