...
async function fetchPrefs(db: any, userId: string): Promise<ProfilePrefs | null> {
  const { data } = await db
    .from('profiles')
    .select('id,notif_daily,notif_health,notif_appointment,notif_lost,notif_family,notif_playdate,notif_chat,notif_event,notif_mention,quiet_hours_enabled,quiet_hours_start,quiet_hours_end,timezone')
    .eq('id', userId)
    .single();
  return data ?? null;
}
