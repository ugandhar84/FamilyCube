-- Live QA finding (docs/qa_reassign_takeover_audit.html, High): the latest
-- decline_event_assignment (20260930360000) nulls driver_name/helper_name
-- and their *_status on decline, but never clears driver_id/helper_id — a
-- regression from an earlier migration (20260930250000) that had this
-- correct. A stale id surviving a decline can still match a real member in
-- any id-based "is this mine" check (classifyEventUrgency.ts,
-- deriveEventActions' isSelfAssigned) even though the display fields
-- correctly show nobody assigned — the exact "conflicting/stale field"
-- bug class already fixed today in forkRideLegs and reassign_event.
